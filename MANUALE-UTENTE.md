...
| **Import ShipFinder** (toggle) | Recupera dati da ShipFinder e — unica tra le fonti gratuite — la **posizione dell'ultimo avvistamento**, usata per ri-localizzare le navi seguite che l'AIS non vede più (in automatico, in background) e tramite il pulsante **📍 Localizza via ShipFinder** nel dettaglio. Le posizioni appaiono come marker arancioni distinti, senza alterare traccia AIS/score/replay. Dati statici in cache per 6 ore. Spento di default. |
