...
### MyShipTracking data (second backup source)

**Import MyShipTracking** works exactly like ShipFinder and serves as a **second, independent backup** for re-locating lost followed ships. With it enabled, the detail view shows a **MyShipTracking data** panel with static fields plus the vessel's **last-seen position**, and the same behaviours: automatic background re-acquisition of stale follows, a **📍 seen on MyShipTracking · <date>** badge, a **📍 Locate via MyShipTracking** button, and **⧉ MyShipTracking** to open the vessel's page. On the mini-map its last-known markers are **teal/cyan**, to distinguish them from ShipFinder's amber.
...
| **Import ShipFinder** (toggle) | Fetch data from ShipFinder and the **last-seen position**, used to re-locate followed ships AIS can no longer see (automatically, in the background) and via the **📍 Locate via ShipFinder** button in the detail view. Positions appear as distinct amber markers, without altering the AIS track/score/replay. Static data cached for 6 hours. Off by default. |
| **Import MyShipTracking** (toggle) | Same as Import ShipFinder, as a **second, independent backup** source for the last-seen position (terrestrial AIS: strong near coasts/ports). Background re-acquisition + **📍 Locate via MyShipTracking** button; positions appear as distinct teal/cyan markers. Static data cached for 6 hours. Off by default. |
...
| `IMPORT_MST_DATA` | `true`/`false` — enable MyShipTracking import: second position-backup source, same role as ShipFinder |
