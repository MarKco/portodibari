...
- **Fallback mode** — per area, automatic (on prolonged AIS silence) or manually forced (for spots with structurally thin AIS coverage); switches to scraping-based position recovery and new-ship discovery via port arrivals, with a shared hourly request budget, source rotation and a circuit breaker keeping scrape volume ban-safe.
