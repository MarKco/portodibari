...
[`src/services/fallback-mode.js`](../../src/services/fallback-mode.js) is **no longer a global switch**. Each monitored area is independently "in silent fallback" based on two facts on its `areas` row: `fallback_enabled` (bool, default 1, admin-configurable per area from the **Areas** screen) and `last_ais_message_at` (timestamp, updated by [`ais-stream.js`](../../src/services/ais-stream.js) on every real AIS message attributed to that area, throttled to 1 write/minute per area in memory — never a per-message write, so as not to repeat the exact event-loop-starvation bug already hit and fixed for berth recompute, see below). "Area X is silent right now" is **computed on the fly** on every sweep — `fallback_enabled AND (last_ais_message_at IS NULL OR older than AREA_SILENT_THRESHOLD_MIN)` — never persisted as state: no `enter()`/`exit()`, no exit hysteresis needed (it's silent and low-risk, self-corrects on the first real message).
...
- **Admin panel and UI, all admin-only** — the per-area toggle lives on the **Areas** screen (`PATCH /areas/:key/fallback`, ⚠ badge if the port isn't resolved on MST — repositioning already-known ships still works, only new-ship discovery is inactive). Settings → **AIS Diagnostics** shows a per-area list (silent yes/no, since when) instead of the old single global block, plus the circuit breaker and scrape-volume history (aggregated, not per-area) — the "followed-only vs full" comparison is gone (it's no longer a binary global choice). The server strips `fallbackEnabled`/`portArrivalsReady`/the per-area breakdown from `GET /areas` and `GET /api/stream/status`/`health` for non-admins (`GET /api/stream/health` is now `requireAdmin`). The **"Ships present"** page shows the current area's name plus a 📡/🔀 badge (admin only) telling whether that area's data right now comes from AIS or from fallback.
...
- **Minimal migration, backup verified** — no new table: just two columns on `areas` (`fallback_enabled INTEGER DEFAULT 1`, `last_ais_message_at TEXT`, via the usual `ALTER TABLE ... ADD COLUMN` try/catch loop), and `areas` is already in `BACKUP_TABLES`. Empirically verified (not just by construction) that a pre-feature backup, lacking these columns, restores with `fallback_enabled=1`/`last_ais_message_at=NULL` — exactly the safe defaults, thanks to how `restoreFrom` copies by column intersection. The old `meta` rows `fallback_mode_active`/`fallback_mode_since` from a pre-feature backup are left dangling and harmless: nothing reads them anymore.
