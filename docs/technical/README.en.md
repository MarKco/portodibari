...
- Otherwise, a 4-source cascade (every available source is queried, it doesn't stop at the first one that answers):
  ...
  - `mst_pid` (MyShipTracking's own internal port id) is resolved lazily, **only for already-`confirmed` ports**, via `searchPort`/`getPortCoords` in `src/services/scrapers/myshiptracking.js` — the field `sweepPortDiscovery()` in `fallback-mode.js` uses to query the port's arrivals/departures (see [Fallback mode](#-fallback-mode-per-area-silent)): written here, consumed there.
...
**Admin routes** (`src/routes/areas.js`, all `requireAdmin`): `GET /areas/:key/ports`, `POST /areas/:key/discover-ports` (fire-and-forget, re-runs the cascade above), `POST /areas/:key/ports/:id/confirm`, `POST /areas/:key/ports/:id/reject` — plus, for comparing against external sources (below), `GET /areas/:key/ports/search-external/stream` (SSE) and `POST /areas/:key/ports/candidates` (manual add, single or batch).

**UI — Ports panel, Areas screen (`⚓` per row)**: this feature's first version shipped with no UI (removed 2026-08-22, before the berth→port granularity fix, as premature) — reintroduced the same day, after the fix, plus compare-against-external-sources on top. `public/js/areas.js` `openPortsPanel`: an **inline** panel below the Areas map (not a modal — the map with its highlighted markers stays visible while reading the list), a table with name/coordinates/status/sources and confirm/reject buttons for `review` rows, a "Search ports now" button (re-runs `discover-ports`). A separate **"Compare against external sources"** section: queries the 4-source cascade **independently** of the berths shortcut (useful even when the area already has observed berths, unlike automatic discovery) and shows raw, unclustered results **live** as they arrive (see `searchExternalCandidates` under [Fallback mode](#-fallback-mode-per-area-silent) for the streaming/add technical details). `area_ports` rows still get populated silently in the background even without ever opening this panel.
...
| Minutes of AIS silence before an area is "in silent fallback" | `app.config.properties` | `AREA_SILENT_THRESHOLD_MIN` | 20 min |
...
- **Markers on the overview map** (`renderActiveMap` in `public/js/maps.js`): color by risk band — 🟢 green (`low`) · 🟡 yellow (`med`) · 🔴 red (`high`). **Flagged** (★ `flagged`) ships take priority and are colored **🟣 purple**, both as markers and as list rows (`.flagged-row`). Auto-detected military ships retain red even when flagged. The marker popup shows the risk badge.
  - **Bug fixed — switching area didn't re-center the map**: `renderActiveMap` frames the area's bbox (not the ships) only when it changes from the last framing (`activeFitKey`, so it doesn't fight the user's own pan/zoom on every poll) — but that check lived **after** an early return for "no positioned ships", so switching to an area with zero ships at that moment left the map stuck on the previous view until the first position arrived. The sidebar **Area** dropdown does call `fitBounds` directly and independently too (`initBboxSelect` in `main.js`), so the symptom only showed when that direct call couldn't fire yet (map not initialized) and the next poll had to carry it alone. Fix: the framing block now runs **before** the early return, in a shared function (`fitActiveAreaBbox`).
...
**Compute cycle** — one-shot *backfill* at startup (`berths.recomputeAll()` in `src/server.js`, idempotent) over all history, then two distinct, non-interchangeable paths:

- **`recomputeAreaFull`** — a complete reset (`clearMooringBerths` + re-cluster from scratch over the area's *entire* mooring history). Correct by construction (handles any manual edit, e.g. a resized polygon that must be able to reclaim points from a different berth), but its cost grows with the whole history — reserved for rare, explicit admin actions: create/edit/merge/delete a berth, the "recompute now" button, post-restore.
- **`recomputeAreaIncremental`** — used by the periodic timer (`BERTH_RECOMPUTE_MIN`) and the dirty-flush (`markAreaDirty`/`BERTH_DIRTY_FLUSH_MIN`, triggered by `ais-stream.js` on every arrival/departure). Touches **only** moorings with no berth yet, or ones belonging to a ship still in port whose centroid just moved — never the whole history.

**A bug fixed, in two stages**: the very first version of `recomputeAreaIncremental` still called `syncMoorings(area)` — which itself re-reads the area's **entire** arrival/mooring history (`getArrivalsForArea`/`getMoorings`, no time filter) to figure out what needs recomputing — on every single cycle. On a long-lived, busy area (Israele, ~16k arrivals/13k moorings in a real export) this alone was measured as 100% of the cost of a **213-second** cycle, blocking the event loop long enough to drop the AIS WebSocket connection (same symptom, different cause, as the historical full-rebuild bug already fixed). Two-part fix:
  1. **`syncMooringsIncremental`** (new, used only by the incremental path) replaces the full scan with `db.getDirtyArrivalsForArea` — a query returning only arrivals that are **genuinely new** (no matching `moorings` row) or **the latest arrival for each mmsi** (a ship "still in port" by `getArrivalsForArea`'s own convention, the only case whose centroid can have changed) — plus `db.getMooringsForMmsis` for a moorings fetch bounded to just those mmsis instead of the whole area. Verified with a direct comparison on real data (same export): **byte-identical** result to the old algorithm (same 222 rows touched, same lat/lon) — just far more targeted. The original `syncMoorings` is unchanged, still used by `recomputeAreaFull` where the full scan is genuinely needed.
  2. **Missing index** — `getStayCentroid` (the aggregate query computing a stay's centroid) filtered on `mmsi`/`area`/`received_at` range, but the only available index covered `area` alone: every call scanned **all** of the area's readings, not just that one ship's. Added `idx_readings_mmsi_area_time ON readings(mmsi, area, received_at)` (verified via `EXPLAIN QUERY PLAN`: from `SEARCH readings USING INDEX idx_readings_area` to `SEARCH readings USING INDEX idx_readings_mmsi_area_time` seeking on all three columns) — on the same sync pass, **13.5s → 90ms**. Also added `idx_moorings_area_mmsi_ts ON moorings(area, mmsi, ts)` for the new query above. No data migration: just `CREATE INDEX IF NOT EXISTS`, free on an old backup (the columns already exist).
  3. **Cooperative yielding** — `syncMoorings`/`syncMooringsIncremental` now yield to the event loop (`setImmediate`) every 200 rows processed, so even an exceptionally long cycle (a backlog after a restore, not a steady state) degrades to small chunks instead of blocking the process for minutes; re-entrancy guards (`flushing`/`recomputeAllRunning`) stop two cycles from overlapping if one runs longer than its own interval.
...
- **Automatic sweep.** Every stale followed ship (no fresh AIS position, including ones **never located** — followed by search, via `getAllFollowedShips`) is scraped by [`fallback-mode.js`](../../src/services/fallback-mode.js)'s own periodic sweep (see [Fallback mode](#-fallback-mode-per-area-silent)) — **not** a `ship-follow.js`-owned loop: that module kept only its WS/subscription/worldwide-box logic, all SF/MST scraping (followed **and** area ships) now lives in one place so they share a single anti-ban budget. Oldest-scraped-first priority and a shared hourly cap (`FALLBACK_MAX_REQ_PER_HOUR`) replace the old fixed per-sweep timer/cap, jittered 1.5–4.5s between requests, negative-cache on failures → low volume / captcha-safe.
...
**MyShipTracking** replicates this exact mechanism as a **second, independent backup**: tagged storage `source='mst'` (same exclusion from track/score/replay), same sweep in `fallback-mode.js`, **📍 Locate via MyShipTracking** button (`POST /api/ships/:mmsi/mstlocate`). When both sources are enabled a stale ship is queried on both (rotated per cycle, not both at once); on the map MST markers are **teal/cyan** to tell them apart from ShipFinder's amber.
...
| **Badge / fallback logic** | `src/routes/ships.js` → `scrapeBadgeAt()`, `scrapeFallbackFix()` | A scraped fix is surfaced (badge or marker) only when AIS is stale **and** the scrape is newer; a fresh AIS fix hides it. Used by both `/ships/followed/active` and `/ships/active` (area ships). |
...
| **Stale re-acquire via SF/MST** | `src/services/fallback-mode.js` → `sweep()`/`scrapeOne()` | Shared periodic sweep (see [Fallback mode](#-fallback-mode-per-area-silent)): scrapes stale follows **and**, for currently-silent areas, area ships — stores fixes with `source='sf'/'mst'`. |
...
[`src/services/fallback-mode.js`](../../src/services/fallback-mode.js) is **no longer a global switch**. Each monitored area is independently "in silent fallback" based on three facts on its `areas` row: `fallback_enabled` (bool, default 1, admin-configurable per area from the **Areas** screen — master on/off), `last_ais_message_at` (timestamp, updated by [`ais-stream.js`](../../src/services/ais-stream.js) on every real AIS message attributed to that area, throttled to 1 write/minute per area in memory — never a per-message write, so as not to repeat the exact event-loop-starvation bug already hit and fixed for berth recompute, see below) and `fallback_forced` (bool, default 0, admin-configurable per area — see below). "Area X is silent right now" is **computed on the fly** on every sweep — `fallback_enabled AND (fallback_forced OR last_ais_message_at IS NULL OR older than AREA_SILENT_THRESHOLD_MIN)` — never persisted as state: no `enter()`/`exit()`, no exit hysteresis needed (it's silent and low-risk, self-corrects on the first real message, unless forced).

This is **deliberately decoupled** from the global AIS outage detector described above ([`ais-uptime.js`](../../src/services/ais-uptime.js), unchanged): that detector still only **notifies** (Telegram/in-app "AIS outage" to every linked user, about the whole AISStream service) — it no longer switches anything on or off. An area can be in silent fallback due to structurally poor/absent AISStream coverage (not an outage, a physical limit of the source for that spot) without anyone getting a notification: that's the feature's explicit use case.

- **Followed ships, handled unconditionally** — `fallback-mode.js` covers SF/MST scraping for **every** stale followed ship on each sweep, independent of any area (a followed ship doesn't belong to a single area). [`ship-follow.js`](../../src/services/ship-follow.js) lost its own scraping branch (`refresh()`'s `reacquireStaleViaShipfinder`/`reacquireStaleViaMst` are gone) — it keeps only its dedicated WS/per-ship subscription/worldwide box, which is free AIS re-acquisition, not scraping. Reason for the merge: a single shared anti-ban budget by design (see below) — if the two modules scraped the same ship in parallel, the "shared" cap would effectively double.
- **Area ships** — `db.getStaleAreaShips(freshMs, areaSilentCutoffIso)` joins `ships.last_area = areas.key`: only ships whose area is currently silent **and** enabled enter the pool. Deliberately does not reuse `ACTIVE_PREDICATE` (unlike `getActiveShips`): that predicate is exactly what demotes a ship from "active" to "past" after `ACTIVE_WINDOW_HOURS`/`PORT_WINDOW_HOURS` of silence — reusing it here would create a catch-22 for any outage longer than that window (a ship needs a scraped fix to look "active" again, but it's excluded from scraping the moment it stops looking active) that permanently blocks repositioning — a real bug observed in production (AIS down for days, scraping active, empty map) and the origin of this whole redesign.
- **Discovering ships AIS never reported, via port arrivals/departures** — for a silent area with at least one **confirmed** port **resolved on MyShipTracking** (`area_ports.status='confirmed' AND mst_pid IS NOT NULL` — the resolution already runs at boot/area create/edit via `resolveMstPidForConfirmedPorts`, in `port-discovery.js`, previously written but never consumed), `sweepPortDiscovery()` calls `crawlPortArrivals(pid)` (`scrapers/myshiptracking.js`, previously dead code with zero callers) on its own cadence of 30min/port (in-memory map `lastPortPollAt`, not in the DB — same reasoning as the `last_ais_message_at` throttle). For each never-seen `{mmsi, name}`: `db.ensureShipStub` (the same primitive already used for followed ships discovered only via scraping) + `db.setShipLastArea` + `enrichment.enrichNewShip`. **No position scrape happens here**: the stubbed ship (sentinel `NEVER_SEEN_AIS`) enters the normal reposition pool by itself on the next pass — zero double-counting against the budget, `scrapeOne()` stays unchanged. An intrinsic, not software-fixable limit: this only works for areas with a recognizable port in the bbox (true almost always for this app's areas) — it can't discover open-water ships with no port, and it can't "invent" an MMSI that was never seen anywhere. No integration does bbox-based vessel search (verified by reading the code: VF/MT/GFW/SF/MST all require an already-known MMSI/IMO; GFW's bbox function, `getAnchoragesInBbox`, is a dead stub — confirmed 404/403 against the public API with the current token) — `crawlPortArrivals` is the one exception, and it's per-port, not per-bbox.
- **VF/MT suspended, per-ship/per-area** — [`enrichment.js`](../../src/services/enrichment.js) suspends VesselFinder/MarineTraffic (`enrichNewShip`, `enrichAllExisting`, `enrichActiveShips`) only for ships whose **own** area is currently silent (`isAreaSilentForShip(ship)`), no longer via a global switch: a ship in a healthy area keeps getting VF/MT even while a DIFFERENT area is silent at the same time. They carry no coordinates (a paid feature) and MarineTraffic is the most fragile source (Cloudflare bypass via `node-libcurl`); the check duplicates the silence logic instead of calling into `fallback-mode.js`, to avoid a require cycle (the port-discovery sweep itself calls `enrichment.enrichNewShip`).
- **Manual per-area force override** — `fallback_forced` (column on `areas`, default 0) makes an area count as "in fallback" even while AIS is sending it messages normally: meant for a bbox with structurally thin real AIS coverage (few transponder-equipped ships), where the silence threshold would never trigger on its own. Only touches `getStaleAreaShips`/`getPortDiscoveryTargets` (SQL, `OR a.fallback_forced = 1`) and `enrichment.js`'s `isAreaSilentForShip` — the area's AIS stream stays always on and listened to, unchanged: forcing adds scraping in parallel, never replaces it. `getAreaStatuses()` keeps `silentSince` (genuine silence only, from `last_ais_message_at`) separate from `fallbackForced` (its own bool) — a forced-but-fresh area has no real "silent since" instant, so the UI doesn't invent one.
- **Admin panel and UI, all admin-only** — two independent toggles per row on the **Areas** screen (`PATCH /areas/:key/fallback`, body `{enabled}` and/or `{forced}`): master on/off and ⚡ force (shown only while the master is on). Live status badge per row — "🟢 AIS active" or "🔴 In fallback" (with a duration when the silence is genuine, "(forced)" otherwise) — plus an ⚠/ℹ badge if the port isn't resolved on MST (repositioning already-known ships still works, only new-ship discovery is inactive). Settings → **AIS Diagnostics** shows the same per-area list instead of the old single global block, plus the circuit breaker and scrape-volume history (aggregated, not per-area). The server strips `fallbackEnabled`/`fallbackForced`/`portArrivalsReady`/the per-area breakdown from `GET /areas` and `GET /api/stream/status`/`health` for non-admins (`GET /api/stream/health` is now `requireAdmin`). The **"Ships present"** page shows the current area's name plus a 📡/🔀 badge (admin only) telling whether that area's data right now comes from AIS or from fallback.
- **Ports panel (Areas screen, `⚓`)** — [`public/js/areas.js`](../../public/js/areas.js) `openPortsPanel`, an **inline** panel below the map (not a modal: the map with its highlighted markers stays visible while reading the list — a first attempt with `#modal-overlay` covered the very map it was meant to show alongside, caught and fixed before release). Port table (`GET /areas/:key/ports`) with confirm/reject (`POST /areas/:key/ports/:id/confirm|reject`) unchanged. **New**: a "Compare against external sources" section, always available (unlike automatic discovery, not gated on whether berths are already observed) — `GET /areas/:key/ports/search-external/stream` (SSE, `requireAdmin`) calls `portDiscovery.searchExternalCandidates(areaKey, onCandidate, shouldStop)`, a locode/WPI/GFW/VesselFinder cascade **extracted** from `discoverPortsForArea` into a reusable function that invokes `onCandidate` for each raw result (not clustered — a human decides here, no need to dedupe) as it arrives — locode/WPI/GFW almost instant, VesselFinder at the existing paced rate (jitter, up to 20 lookups). `shouldStop` (polled between VF lookups) stops burning anti-ban budget if the admin closes the window mid-search. **Deliberately without the `isAnyAreaSilent()` gate** that protects `discoverPortsForArea`: that gate exists only because the full flow also calls `resolveMstPidForConfirmedPorts` (MST, sharing `fallback-mode.js`'s budget) right after — `searchExternalCandidates` alone never touches SF/MST, so gating it the same way would block the feature globally the moment any one area is silent/forced, for a resource it doesn't share. Frontend: a dedicated map layer (dashed blue markers) alongside the confirmed-ports one, a checkbox list populated via `insertAdjacentHTML` (not a full re-render, which would uncheck already-ticked boxes while a search that can run over a minute keeps streaming in more candidates), `POST /areas/:key/ports/candidates` (`{candidates:[{name,lat,lon,source}]}`, max 200) adds straight to `confirmed`/`admin_reviewed=1` via `db.addManualAreaPort` — the admin's pick already is the review, unlike the automatic upsert which lands on `review` when only one source agrees.
- **Bug fixed — slider click swallowed by the row** — the Areas table's click delegation used `e.target.closest('.area-fallback-toggle')` to avoid opening the row editor when a toggle was clicked; but clicking the visible `<span class="toggle-slider">` (a sibling, not an ancestor, of the `<input>` carrying that class) never resolves to an ancestor with that class, so the click slipped past the guard and opened the editor anyway — the toggle looked unresponsive (the action actually fired, but the UI jumped elsewhere right after). Fix: guard the whole `label.toggle` instead of just `.area-fallback-toggle`/`.area-fallback-force-toggle`, so any element clicked inside the label (span, label, or input) is correctly recognized.
- **Cleanup of auto-generated duplicate ports** — `db.deleteUnreviewedBerthPorts(areaKey)`, called by `discoverPortsForArea` before re-clustering berths: deletes that area's `area_ports` rows with exactly `sources = '["berths"]'` and `admin_reviewed = 0`. A real bug fixed: `upsertAreaPort` does `ON CONFLICT(area_key, name)` — if a later cluster picks a different representative name (centroid drifted slightly, one member gained/lost), the old row was never replaced, only left standing alongside the new one; an area re-run several times over its life accumulated dozens of near-duplicate rows an admin never reviewed (observed in production: the same spot saved 3-4 times under different names). Rows manually confirmed/rejected or backed by an external source are never touched. The fallback name for an unnamed berth also had its **hardcoded Italian word stripped** (`Porto {lat},{lon}` → coordinates only): a name persisted in the DB can't be run through the client's i18n at display time.
- **Live call log** ("🔀 Fallback mode log", admin-only, above "Activity log" in the sidebar) — a floating window showing calls to ShipFinder/MyShipTracking in real time as they happen, regardless of which area/ship triggered them. Same mechanism as the operational log (`app-log.js`/`realtime.js`): `fallback-mode.js`'s `scrapeOne()` calls `broadcastFallbackScrape({ts,source,mmsi,ok})` (in `realtime.js`, same SSE-client-Set + `res.write` pattern as `broadcastAppLog`) after every attempt, success or failure; `GET /api/fallback-scrape/stream` (SSE, `requireAdmin`, `routes/fallback-scrape.js`) fans it out to connected clients, `GET /api/fallback-scrape` backfills from an in-memory buffer of the last 200 calls (`fallbackMode.getRecentScrapeEvents()`, same in-memory-only tradeoff as the circuit breaker). Frontend (`public/js/fallback-scrape-log.js`): one `EventSource` per window, an **SVG line chart** (no longer stacked bars — with 30s buckets over a mostly-empty series, bars looked misaligned/random; a line with dot markers on non-zero points reads better) for succeeded/failed with a color legend, bucketed at 30s over the last 10 minutes, recomputed on every event — unlike the 48h historical chart in AIS Diagnostics (hourly buckets, from `scrape_log`, still bars), this one is push-based and fine-grained, meant for "are we scraping right now, and is it working?" not history. The operational-log lines (`Fallback: posizione ShipFinder/MyShipTracking per <ship>`) now also carry **area and nearest port** in brackets whenever the ship belongs to an area (`areaPortLogTag` in `fallback-mode.js`, finds the nearest confirmed port within 8km using the same `haversineM` from `ship-analysis.js`) — absent for followed ships, which have no area.
- **Anti-ban measures, in order of importance** (unchanged — a single shared budget/circuit breaker across followed ships, area ships, **and** port discovery; multiple silent areas at once redistribute the same cap instead of adding to it):
...
- **Minimal migration, backup verified** — no new table: three columns on `areas` (`fallback_enabled INTEGER DEFAULT 1`, `last_ais_message_at TEXT`, `fallback_forced INTEGER DEFAULT 0`, via the usual `ALTER TABLE ... ADD COLUMN` try/catch loop), and `areas` is already in `BACKUP_TABLES`. Empirically verified (not just by construction) that a pre-feature backup, lacking these columns, restores with `fallback_enabled=1`/`last_ais_message_at=NULL`/`fallback_forced=0` — exactly the safe defaults, thanks to how `restoreFrom` copies by column intersection. The old `meta` rows `fallback_mode_active`/`fallback_mode_since` from a pre-feature backup are left dangling and harmless: nothing reads them anymore.
- **`outage.since` comes from an authoritative external source, not local detection** — `probeOne()` also reads `lastMessageReceived` from the uptime monitor's response (the last message the monitor itself saw, not us), used as `since` when available — falling back to `meta` (`ais_outage_since`, persisted to survive restarts) and finally to "now" if neither is available. This way a **first-ever deploy made days into an already-ongoing outage** shows the real hours of downtime right away in the banner/notifications, instead of restarting the countdown from zero on every deploy.
- **Restart mid-outage: banner and verdict without the wait** — `outage.serviceDown`/`monitorState`/`monitorSource` are rehydrated from `meta` at module load (alongside `since`, already persisted), so the banner immediately shows the last known state instead of going blank until `evaluate()` recomputes it. A restart also resets the stream's **local silence** (it reconnects from scratch): without a fix, `evaluate()` would have to wait `AIS_OUTAGE_SILENCE_MIN` minutes of silence all over again before even re-contacting the monitor, leaving the banner blank/stale in the meantime. `init()` therefore calls `reconfirmOnBoot()` once when the rehydrated state says "was down": an immediate probe to the monitor (still respecting `MIN_PROBE_GAP_MS` for every check after it) that either reconfirms the ongoing outage (no new notification — it's not a new detection) or clears it right away if it resolved during the restart. `evaluate()` and `reconfirmOnBoot()` share the same transition logic (`applyDown`/`applyUp`) so the `since` fallback chain can't drift between the two call sites. This logic is **independent** of the per-area fallback mode above — it only concerns the global outage banner/notifications.
...
> **Bug fixed — `data-tip` inert outside the whitelist**: `initGlossaryTooltip()` only wires `mouseover`/`mouseout` on elements matching `TIP_SELECTOR`, an explicit CSS-selector whitelist — a `data-tip` attribute on an element **not included** there is silently ignored, no error, no fallback to the native `title`: it just looks like a tooltip that does nothing. The Areas screen's badges/toggles (the live 🟢/🔴 dot, the ℹ/⚠ port badge, and the two `<label class="toggle">` per row) had been built with `data-tip` following the existing pattern but were **never added** to `TIP_SELECTOR` — invisible on hover since their very first commit, not a regression. Fix: `.area-fallback-live[data-tip], .area-badge-warn[data-tip], .area-badge-info[data-tip], .toggle[data-tip]` added to the whitelist in `main.js`. `.toggle` is generic (used by every on/off switch in the app) but the selector still requires `[data-tip]`, so it only fires where the attribute is actually present — reusable for any future tooltip on a toggle, not just these two.
