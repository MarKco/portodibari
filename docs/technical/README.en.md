...
- **Manual**: admin "Cerca porti ora"/"Search ports now" button on the Areas screen — also fire-and-forget (`POST /api/areas/:key/discover-ports` responds `{ok:true}` immediately; the real work — which can take minutes for a fresh no-berths area — runs in the background; the frontend re-fetches the port list on its own once it's done).
...
**UI**: inside the existing "🗺 Areas" screen, admin-only (the panel isn't rendered at all for non-admin users, client-side gate on `S.isAdmin` plus server-side `requireAdmin`), each area row expands into a ports list with name, status badge (Confirmed/Needs review/Rejected — green/amber/red), the contributing sources, Confirm/Reject buttons (shown only for `review`-status ports) and the "Search ports now" button. Files involved: `public/js/areas.js`, `public/index.html` (7-column table, the ports row spans it), `public/css/style.css`, `public/locales/it.js`+`en.js`.
