...
- **Berths shortcut**: if the area already has real observed berth clusters (`db.getBerths`), those **are** the answer — every berth is auto-confirmed as a port (source `berths`), **no external source is contacted at all**.
...
- **Manual**: admin "Cerca porti ora"/"Search ports now" button on the Areas screen — also fire-and-forget (`POST /api/areas/:key/discover-ports` responds `{ok:true}` immediately; the real work — which can take minutes for a fresh no-berths area — runs in the background). The frontend re-fetches the port list **once, right after** the click — it will almost always still show the unchanged state, since the real cascade can take minutes. There's no polling and no progress indicator: to see the outcome, click "Search ports now" again (or reopen the Areas screen) after a bit.

**Admin routes** (`src/routes/areas.js`, all `requireAdmin`): `GET /areas/:key/ports`, `POST /areas/:key/discover-ports` (fire-and-forget), `POST /areas/:key/ports/:id/confirm`, `POST /areas/:key/ports/:id/reject`.

**UI**: inside the existing "🗺 Areas" screen, admin-only (the panel isn't rendered at all for non-admin users, client-side gate on `S.isAdmin` plus server-side `requireAdmin`), each area row expands into a ports list with name, status badge (Confirmed/Needs review/Rejected — green/amber/red), the contributing sources, and the "Search ports now" button. Confirm/Reject buttons: both shown for `review`-status ports; a port already `confirmed`/`rejected` shows only the opposite action's button (Reject/Confirm) instead, letting an admin correct an earlier decision — the backend (`POST .../confirm`|`reject`) never had a status restriction, only the frontend used to hide the button. Files involved: `public/js/areas.js`, `public/index.html` (7-column table, the ports row spans it), `public/css/style.css`, `public/locales/it.js`+`en.js`.
