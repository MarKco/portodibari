...
| Ore di disservizio AIS prima di entrare in modalità fallback | `app.config.properties` | `AIS_FALLBACK_HOURS` | 6 ore |
| Minuti di servizio stabile prima di uscire dalla modalità fallback | `app.config.properties` | `AIS_FALLBACK_EXIT_GRACE_MIN` | 20 min |
| Budget massimo scraping/ora in modalità fallback (SF+MST insieme) | `app.config.properties` | `FALLBACK_MAX_REQ_PER_HOUR` | 90 |
...
### 🔀 Modalità fallback (disservizio AIS prolungato)

Quando il disservizio AIS (sopra) resta attivo per più di `AIS_FALLBACK_HOURS` (default 6h), [`src/services/fallback-mode.js`](../../src/services/fallback-mode.js) attiva una **modalità fallback**: invece di limitarsi alla ri-localizzazione delle sole navi seguite (il meccanismo SF/MST descritto sopra), intensifica lo scraping per garantire un minimo di monitoraggio anche durante un down prolungato — con precauzioni esplicite per non generare un volume che rischi un ban dalle piattaforme sorgente.

- **Trigger e isteresi** — il modulo non ha una propria logica di rilevamento: riusa `outage.since` di [`ais-uptime.js`](../../src/services/ais-uptime.js) (già la fonte di verità per "disservizio confermato da quando"). All'ingresso: `outage.serviceDown && now - since >= AIS_FALLBACK_HOURS`. All'uscita: non basta un singolo check pulito — serve `AIS_FALLBACK_EXIT_GRACE_MIN` (default 20 min) di servizio **continuativamente** stabile, altrimenti un blip di pochi minuti disattiverebbe e riattiverebbe la modalità inutilmente (il pending-exit si annulla se torna giù prima che il timer scada).
- **Scope: sempre auto-avviato "solo navi seguite"** — `state.fallbackScopeAreas` viene **forzato a `false`** ad ogni `enter()`, indipendentemente da cosa un admin avesse scelto l'ultima volta: la scelta più prudente non è mai un default fisso in config, va sempre confermata a occhio. Un admin può estendere a "monitoraggio completo" (anche navi correnti nelle bbox monitorate, non solo seguite — `db.getStaleAreaShips`) dal pannello **Impostazioni → Diagnostica AIS**, che mostra lo storico reale di scraping (ultime 48h, per ora, da `db.getScrapeCountsHourly`) **affiancato** alla stima richieste/ora delle due modalità (`fallbackMode.getEstimate()`), per decidere col numero in mano invece che a scatola chiusa. Allargare lo scope non moltiplica il budget: lo stesso tetto orario viene ridistribuito su più navi (ognuna rivisitata meno spesso). Il pannello mostra anche la **durata della sessione corrente** (da `fallback_mode_since`) e, per sorgente, un **contatore di sospetti blocco nella sessione** (`tripCounts` in `fallbackMode.getStatus()`, in-memory, azzerato a ogni `enter()`) accanto allo stato circuito aperto/chiuso — un problema intermittente che si è già auto-risolto resta comunque visibile invece di sparire non appena il circuito si richiude. Lo storico a 48h è un **piccolo multiplo per sorgente** (ShipFinder/MyShipTracking separati, non più sommati) con barre impilate riuscite/fallite (colori di stato verde/rosso, non identità) ed etichette orarie ogni 6h. Una voce **in sidebar** ("🔀 Modalità fallback", visibile solo agli admin mentre il fallback è attivo, aggiornata ad ogni poll di `/api/stream/status` come il banner di disservizio) porta dritti a questo pannello da qualsiasi schermata.
- **Log live delle chiamate** ("🔀 Log modalità fallback", stessa condizione di visibilità admin+fallback-attivo, sopra "Log attività" in sidebar) — finestra flottante che mostra, in tempo reale, le chiamate a ShipFinder/MyShipTracking man mano che avvengono. Meccanismo identico al log applicativo (`app-log.js`/`realtime.js`): `fallback-mode.js`'s `scrapeOne()` chiama `broadcastFallbackScrape({ts,source,mmsi,ok})` (nuovo, in `realtime.js`, stesso pattern Set-di-client-SSE + `res.write` di `broadcastAppLog`) dopo ogni tentativo, riuscito o no; `GET /api/fallback-scrape/stream` (SSE, `requireAdmin`, nuova route dedicata `routes/fallback-scrape.js`) fan-out ai client connessi, `GET /api/fallback-scrape` backfilla con un buffer in-memory delle ultime 200 chiamate (`fallbackMode.getRecentScrapeEvents()`, stesso tradeoff in-memory-only del circuit breaker). Frontend (`public/js/fallback-scrape-log.js`): un `EventSource` per finestra, due mini-grafici a barre impilate riuscite/fallite (stessa semantica cromatica di sourceHistoryChart) a bucket di 30s sugli ultimi 10 minuti, ricalcolati ad ogni evento — a differenza del grafico storico a 48h in Diagnostica AIS (bucket orari, dati da `scrape_log`), questo è push-based e a grana fine, pensato per "sto scrapando adesso, sta andando bene?" non per lo storico.
- **Sospensione VF/MT** — durante il fallback, [`enrichment.js`](../../src/services/enrichment.js) sospende **del tutto** l'arricchimento VesselFinder/MarineTraffic (`enrichNewShip`, `enrichAllExisting`, `enrichActiveShips`): non danno coordinate (prodotto a pagamento) quindi non contribuiscono all'obiettivo di continuità posizionale, e MarineTraffic in particolare è la sorgente più fragile (bypass Cloudflare via `node-libcurl`, a rischio di blocco permanente se il volume sale). Tutto il budget di scraping va a SF/MST.
- **`ship-follow.js` cede lo sweep** — il suo `refresh()` (il ciclo periodico che oggi scrapa SF/MST solo per le navi seguite) si disattiva quando `fallbackMode.isActive()`: durante il fallback tutta la logica di scraping (seguite **e**, se abilitato, aree) vive nel nuovo modulo, con budget/priorità/rotazione che il vecchio ciclo non aveva.
- **Anti-ban, in ordine di importanza**:
...
- **Zero migrazioni di schema** — nessuna tabella nuova: il flag `fallback_mode_active`/`fallback_mode_since` vive in `meta` (già in `BACKUP_TABLES`), le query di scoperta navi (`getStaleAreaShips`, `getAdminUserIds`, `getScrapeCountsHourly`) leggono tabelle già esistenti. Un backup pre-feature non ha semplicemente quelle chiavi `meta` — `db.getMeta` le tratta come assenti (default OFF), quindi un restore vecchio funziona senza alcun accorgimento speciale.
- **`outage.since` da fonte esterna autorevole, non da rilevazione locale** — `ais-uptime.js` non conta le `AIS_FALLBACK_HOURS` dal momento in cui *questa istanza* rileva per prima il disservizio: `probeOne()` legge anche `lastMessageReceived` dalla risposta del monitor di uptime (l'ultimo messaggio visto **dal monitor**, non da noi), usato come `since` quando disponibile — fallback su `meta` (`ais_outage_since`, persistito per sopravvivere ai riavvii) e infine su "adesso" se nessuno dei due è disponibile. Così un **primo deploy fatto giorni dentro un disservizio già in corso** calcola da subito le ore di down reali, invece di far ripartire il conto da zero a ogni deploy.
- **Riavvio a metà disservizio: banner e verdetto senza attesa** — `outage.serviceDown`/`monitorState`/`monitorSource` sono ora rehydratati da `meta` all'avvio del modulo (oltre a `since`, già persistito), quindi il banner mostra subito l'ultimo stato noto invece di sparire finché `evaluate()` non lo ricalcola. Un riavvio azzera anche il **silenzio locale** dello stream (si riconnette da zero): senza correttivo, `evaluate()` avrebbe dovuto aspettare di nuovo `AIS_OUTAGE_SILENCE_MIN` minuti di silenzio prima anche solo di ricontattare il monitor — ritardo che, in un riavvio proprio mentre `AIS_FALLBACK_HOURS` sta per scattare, avrebbe rinviato indebitamente l'ingresso in modalità fallback. `init()` chiama quindi `reconfirmOnBoot()` una tantum quando lo stato rehydratato dice "era giù": un probe immediato al monitor (rispetta comunque `MIN_PROBE_GAP_MS` per i check successivi) che conferma l'outage in corso (nessuna nuova notifica, non è una nuova rilevazione) o lo chiude subito se si era risolto durante il riavvio. `evaluate()` e `reconfirmOnBoot()` condividono la stessa logica di transizione (`applyDown`/`applyUp`) per non far divergere la catena di fallback di `since` tra i due punti di chiamata.
