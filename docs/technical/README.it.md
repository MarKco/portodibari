...
**Rotte admin** (`src/routes/areas.js`, tutte `requireAdmin`): `GET /areas/:key/ports`, `POST /areas/:key/discover-ports` (fire-and-forget), `POST /areas/:key/ports/:id/confirm`, `POST /areas/:key/ports/:id/reject`. Le ultime tre restano disponibili ma **non sono esposte da nessuna UI attuale** — i porti scoperti sono oggi solo dato di backend, senza una schermata che ne mostri l'elenco/stato (vedi nota sotto).

**UI**: **nessuna al momento oltre al controllo manuale in Diagnostica AIS**. La prima versione di questa feature esponeva anche, nella schermata "🗺 Aree", un elenco porti per area con badge di stato e bottoni conferma/rifiuta — rimosso (2026-08-22) perché prematuro: quella UI esisteva ancora prima di correggere la granularità banchina→porto, e i porti scoperti non hanno oggi un consumatore reale che ne giustifichi la gestione manuale in questa fase (arriverà con la scoperta nuovi arrivi via pagine porto, non ancora implementata). Le righe `area_ports` restano comunque popolate silenziosamente in background per quando servirà.
...
[`src/services/fallback-mode.js`](../../src/services/fallback-mode.js) **non è più un interruttore globale**. Ogni area monitorata è indipendentemente "in fallback silenzioso" in base a due soli fatti sulla riga `areas` corrispondente: `fallback_enabled` (bool, default 1, admin-configurabile per-area dalla schermata **Aree**) e `last_ais_message_at` (timestamp, aggiornato da [`ais-stream.js`](../../src/services/ais-stream.js) ad ogni messaggio AIS reale attribuito a quell'area, throttlato a 1 scrittura/minuto per area in memoria — mai una scrittura per-messaggio, per non ripetere l'esatto bug di starvation dell'event loop già preso e risolto per il ricalcolo banchine, vedi sotto). "L'area X è silenziosa ora" si **calcola al volo** ad ogni sweep — `fallback_enabled AND (last_ais_message_at IS NULL OR più vecchio di AREA_SILENT_THRESHOLD_MIN)` — mai persistito come stato: niente `enter()`/`exit()`, niente isteresi di uscita (non serve: è silenzioso e a basso rischio, si autocorregge al primo messaggio reale).
...
- **Pannello admin e UI, tutto admin-only** — il toggle per-area vive nella schermata **Aree** (`PATCH /areas/:key/fallback`, badge ⚠ se il porto non è risolto su MST — il riposizionamento delle navi già note funziona comunque, solo la scoperta di navi nuove non è attiva). Impostazioni → **Diagnostica AIS** mostra la lista per-area (silenziosa sì/no, da quando) al posto del vecchio blocco unico globale, più circuit breaker e storico volume scraping (aggregati, non per-area) — sparito il confronto "solo seguite vs completo" (non è più una scelta binaria). Il server filtra `fallbackEnabled`/`portArrivalsReady`/il dettaglio per-area sia su `GET /areas` sia su `GET /api/stream/status`/`health` per i non-admin (`GET /api/stream/health` ora `requireAdmin`). La pagina **"Navi presenti"** mostra il nome dell'area corrente + un badge 📡/🔀 (solo admin) che indica se i dati di quell'area vengono da AIS o da fallback in questo momento.
- **Log live delle chiamate** ("🔀 Log modalità fallback", visibile solo agli admin, sopra "Log attività" in sidebar) — finestra flottante che mostra, in tempo reale, le chiamate a ShipFinder/MyShipTracking man mano che avvengono, indipendentemente da quale area/nave le ha innescate. Meccanismo identico al log applicativo (`app-log.js`/`realtime.js`): `fallback-mode.js`'s `scrapeOne()` chiama `broadcastFallbackScrape({ts,source,mmsi,ok})` (in `realtime.js`, stesso pattern Set-di-client-SSE + `res.write` di `broadcastAppLog`) dopo ogni tentativo, riuscito o no; `GET /api/fallback-scrape/stream` (SSE, `requireAdmin`, `routes/fallback-scrape.js`) fan-out ai client connessi, `GET /api/fallback-scrape` backfilla con un buffer in-memory delle ultime 200 chiamate (`fallbackMode.getRecentScrapeEvents()`, stesso tradeoff in-memory-only del circuit breaker). Frontend (`public/js/fallback-scrape-log.js`): un `EventSource` per finestra, due mini-grafici a barre impilate riuscite/fallite (stessa semantica cromatica di sourceHistoryChart) a bucket di 30s sugli ultimi 10 minuti, ricalcolati ad ogni evento — a differenza del grafico storico a 48h in Diagnostica AIS (bucket orari, dati da `scrape_log`), questo è push-based e a grana fine, pensato per "sto scrapando adesso, sta andando bene?" non per lo storico.
