...
- **Manuale**: bottone admin "Cerca porti ora"/"Search ports now" nella schermata Aree — anch'esso fire-and-forget (`POST /api/areas/:key/discover-ports` risponde `{ok:true}` subito; il lavoro vero, che su un'area fresca senza banchine può richiedere minuti, gira in background). Il frontend ri-interroga la lista porti **una sola volta, subito dopo** il click — quasi sempre mostrerà ancora lo stato invariato, dato che la cascata reale può richiedere minuti. Non c'è polling né un indicatore di avanzamento: per vedere l'esito bisogna ricliccare "Cerca porti ora" (o riaprire la schermata Aree) dopo un po'.
...
**UI**: dentro la schermata "🗺 Aree" esistente, admin-only (il pannello non viene renderizzato affatto per utenti non-admin, gate lato client su `S.isAdmin` più `requireAdmin` lato server), ogni riga area si espande in un elenco porti con nome, badge di stato (Confermato/Da rivedere/Rifiutato — verde/ambra/rosso), le fonti contribuenti e il bottone "Cerca porti ora". Bottoni Conferma/Rifiuta: entrambi per i porti in stato `review`; per un porto già `confirmed`/`rejected` viene mostrato il solo bottone dell'azione opposta (Rifiuta/Conferma), per permettere all'admin di correggere una decisione precedente — il backend (`POST .../confirm`|`reject`) non ha mai avuto una restrizione di stato, era solo il frontend a non esporre il pulsante. File coinvolti: `public/js/areas.js`, `public/index.html` (tabella a 7 colonne, la riga porti la occupa tutta), `public/css/style.css`, `public/locales/it.js`+`en.js`.
