...
- **Scorciatoia banchine**: se l'area ha già cluster di banchine reali (`db.getBerths`), quelli **sono** la risposta — nessuna fonte esterna viene interrogata. Le banchine vengono prima clusterizzate per prossimità geografica (stessa `clusterCandidates`/`haversineM` della cascata sotto, raggio ~4km) e **ogni cluster diventa una riga porto** (fonte `berths`), non una riga per banchina: un porto reale è un gruppo di banchine vicine, non una banchina singola (corretto dopo la prima implementazione, che sbagliava questa granularità).
- Altrimenti, cascata a 4 fonti (tutte quelle disponibili vengono interrogate, non ci si ferma alla prima che risponde):
  ...
  - `mst_pid` (l'id porto interno di MyShipTracking) viene risolto in modo pigro, **solo per i porti già `confirmed`**, tramite `searchPort`/`getPortCoords` in `src/services/scrapers/myshiptracking.js` — è il campo che `sweepPortDiscovery()` in `fallback-mode.js` usa per interrogare gli arrivi/partenze del porto (vedi [Modalità fallback](#-modalità-fallback-per-area-silenziosa)): scritto qui, consumato lì.
...
- **Manuale**: un controllo minimale in Impostazioni → Diagnostica AIS (select area + bottone "Cerca porti ora"), fire-and-forget (`POST /api/areas/:key/discover-ports` risponde `{ok:true}` subito; il lavoro vero, che su un'area fresca senza banchine può richiedere minuti, gira in background — il controllo mostra solo un messaggio "Ricerca avviata", nessun polling/esito).

**Rotte admin** (`src/routes/areas.js`, tutte `requireAdmin`): `GET /areas/:key/ports`, `POST /areas/:key/discover-ports` (fire-and-forget, rilancia la cascata sopra), `POST /areas/:key/ports/:id/confirm`, `POST /areas/:key/ports/:id/reject` — più, per il confronto con sorgenti esterne (sotto), `GET /areas/:key/ports/search-external/stream` (SSE) e `POST /areas/:key/ports/candidates` (aggiunta manuale, singola o in batch).

**UI — pannello Porti, schermata Aree (`⚓` per riga)**: la prima versione di questa feature era stata rilasciata senza UI (rimossa il 2026-08-22, prima di correggere la granularità banchina→porto, perché prematura) — reintrodotta lo stesso giorno, dopo il fix, con in più il confronto sorgenti esterne. `public/js/areas.js` `openPortsPanel`: pannello **inline** sotto la mappa Aree (non un modale — la mappa con i marker evidenziati resta visibile mentre si legge la lista), tabella con nome/coordinate/stato/sorgenti e bottoni conferma/rifiuta per le righe `review`, bottone "Cerca porti ora" (rilancia `discover-ports`). Sezione separata **"Confronta con sorgenti esterne"**: interroga la cascata a 4 fonti **indipendentemente** dalla scorciatoia banchine (utile anche quando l'area ha già banchine osservate, a differenza della scoperta automatica) e mostra i risultati **grezzi, non clusterizzati, in tempo reale** man mano che arrivano (vedi `searchExternalCandidates` in [Modalità fallback](#-modalità-fallback-per-area-silenziosa) per i dettagli tecnici di streaming/aggiunta). Le righe `area_ports` restano comunque popolate silenziosamente in background anche senza mai aprire questo pannello.
...
| Default cooldown scrape SF/MST per nave (impostabile a runtime, vedi Impostazioni) | `app.config.properties` | `SF_REACQUIRE_THROTTLE_MIN` | 60 min |
| Negative cache scraping VF/MT   | `app.config.properties`           | `SCRAPE_NEG_CACHE_DAYS`               | 3 giorni       |
| Minuti di silenzio AIS prima che un'area sia "in fallback silenzioso" | `app.config.properties` | `AREA_SILENT_THRESHOLD_MIN` | 20 min |
| Budget massimo scraping/ora in modalità fallback (SF+MST insieme, tutte le aree) | `app.config.properties` | `FALLBACK_MAX_REQ_PER_HOUR` | 90 |
| Circuit breaker: fallimenti 403/429 per aprire il circuito | `app.config.properties` | `FALLBACK_CIRCUIT_TRIP_COUNT` | 5 |
| Circuit breaker: finestra temporale dei fallimenti | `app.config.properties` | `FALLBACK_CIRCUIT_TRIP_WINDOW_MIN` | 10 min |
| Circuit breaker: durata cooldown sorgente sospesa | `app.config.properties` | `FALLBACK_CIRCUIT_COOLDOWN_MIN` | 30 min |
...
- **Marker sulla mappa overview** (`renderActiveMap` in `public/js/maps.js`): colore per fascia di rischio — 🟢 verde (`low`) · 🟡 giallo (`med`) · 🔴 rosso (`high`). Le navi **segnalate** (★ `flagged`) hanno priorità e sono colorate **🟣 viola**, sia come marker sia come riga di lista (`.flagged-row`). Le navi militari auto-rilevate mantengono il rosso anche se `flagged`. Il popup del marker mostra il badge di rischio.
  - **Bug corretto — cambio area non ricentrava la mappa**: `renderActiveMap` inquadra il bbox dell'area (non le navi) solo quando cambia rispetto all'ultimo inquadramento (`activeFitKey`, per non combattere pan/zoom dell'utente ad ogni poll) — ma questo controllo viveva **dopo** un early-return per "nessuna nave posizionata", quindi passare a un'area con zero navi al momento del cambio lasciava la mappa bloccata sulla vista precedente finché non arrivava la prima posizione. Il cambio dal menu **Area** in sidebar chiama comunque `fitBounds` in modo diretto e indipendente (`initBboxSelect` in `main.js`), quindi il sintomo si vedeva solo quando quella chiamata diretta non poteva ancora avvenire (mappa non ancora inizializzata) e il poll successivo doveva sopperire da solo. Fix: il blocco di inquadramento gira ora **prima** dell'early-return, in una funzione condivisa (`fitActiveAreaBbox`).
...
**Ciclo di calcolo** — *backfill* una tantum all'avvio (`berths.recomputeAll()` in `src/server.js`, idempotente) su tutto lo storico, poi due percorsi distinti, non intercambiabili:

- **`recomputeAreaFull`** — reset completo (`clearMooringBerths` + re-cluster da zero su *tutta* la storia moorings dell'area). Corretto per costruzione (gestisce qualunque modifica manuale, es. un poligono ridimensionato che deve poter reclamare punti da un'altra banchina), ma il suo costo cresce con l'intera storia — riservato alle azioni admin esplicite e rare: crea/modifica/unisci/elimina banchina, bottone "ricalcola ora", post-restore.
- **`recomputeAreaIncremental`** — usata dal timer periodico (`BERTH_RECOMPUTE_MIN`) e dal dirty-flush (`markAreaDirty`/`BERTH_DIRTY_FLUSH_MIN`, scattato da `ais-stream.js` su ogni arrivo/partenza). Tocca **solo** i moorings senza banchina o quelli di navi ancora in porto il cui centroide si è appena spostato — mai l'intera storia.

**Bug corretto, in due tempi**: la primissima versione di `recomputeAreaIncremental` chiamava comunque `syncMoorings(area)` — che a sua volta rilegge **tutta** la storia arrivi/moorings dell'area (`getArrivalsForArea`/`getMoorings`, nessun filtro temporale) per individuare cosa serve ricalcolare — ad ogni singolo ciclo. Su un'area lunga-vita e trafficata (Israele, ~16k arrivi/13k moorings in un export reale) questo *da solo* è stato misurato come il 100% del costo di un ciclo da **213 secondi**, bloccando l'event loop abbastanza a lungo da far cadere la connessione WebSocket AIS (stesso sintomo, causa diversa, del bug storico sul full-rebuild già risolto). Fix in due parti:
  1. **`syncMooringsIncremental`** (nuova, usata solo dal percorso incrementale) sostituisce la scansione completa con `db.getDirtyArrivalsForArea` — una query che restituisce solo gli arrivi **genuinamente nuovi** (nessuna riga `moorings` corrispondente) o **l'ultimo arrivo per ogni mmsi** (nave "ancora in porto" per convenzione di `getArrivalsForArea`, unico caso il cui centroide può essere cambiato) — più `db.getMooringsForMmsis` per un fetch dei moorings esistenti limitato a quei soli mmsi invece dell'intera area. Verificato con un confronto diretto sui dati reali (stesso export): risultato **byte-identico** al vecchio algoritmo (stesse 222 righe toccate, stessi lat/lon) — solo molto più mirato. `syncMoorings` originale resta invariata, usata solo da `recomputeAreaFull` dove la scansione completa è comunque necessaria.
  2. **Indice mancante** — `getStayCentroid` (la query aggregata per calcolare il centroide di una sosta) filtrava per `mmsi`/`area`/intervallo `received_at`, ma l'unico indice disponibile copriva `area` da solo: ogni chiamata scansionava **tutte** le readings dell'area, non solo quelle della nave. Aggiunto `idx_readings_mmsi_area_time ON readings(mmsi, area, received_at)` (verificato via `EXPLAIN QUERY PLAN`: da `SEARCH readings USING INDEX idx_readings_area` a `SEARCH readings USING INDEX idx_readings_mmsi_area_time` con seek sui tre campi) — sullo stesso giro di sync, **13.5s → 90ms**. Aggiunto anche `idx_moorings_area_mmsi_ts ON moorings(area, mmsi, ts)` per la nuova query di cui sopra. Nessuna migrazione dati: solo `CREATE INDEX IF NOT EXISTS`, gratis su un backup vecchio (le colonne esistono già).
  3. **Yield cooperativo** — `syncMoorings`/`syncMooringsIncremental` ora cedono il controllo all'event loop (`setImmediate`) ogni 200 righe processate, così anche un ciclo eccezionalmente lungo (backlog dopo un restore, non uno stato stabile) degrada a piccoli blocchi invece di bloccare il processo per minuti; guardie di ri-entranza (`flushing`/`recomputeAllRunning`) evitano che due cicli si sovrappongano se uno impiega più del proprio stesso intervallo.
...
- **Sweep automatico.** Ogni nave seguita **stale** (nessuna posizione AIS fresca, incluse quelle **mai localizzate** — seguite da ricerca, via `getAllFollowedShips`) viene scrapata dal sweep periodico di [`fallback-mode.js`](../../src/services/fallback-mode.js) (vedi [Modalità fallback](#-modalità-fallback-per-area-silenziosa)) — **non** più da un loop proprio di `ship-follow.js`, che mantiene solo WS/subscription/world-box: tutto lo scraping SF/MST (seguite **e** navi d'area) vive ora in un solo posto, per condividere un unico budget anti-ban. Priorità alla nave scrapata da più tempo e tetto orario condiviso (`FALLBACK_MAX_REQ_PER_HOUR`) al posto del vecchio timer/tetto fisso per-sweep, jitter 1.5–4.5s tra le richieste, negative-cache sui fallimenti → volume basso/captcha-safe.
...
**MyShipTracking** replica esattamente lo stesso meccanismo come **secondo backup indipendente**: storage taggato `source='mst'` (stessa esclusione da traccia/score, e dal replay salvo toggle **Includi SF/MST**), stesso sweep in `fallback-mode.js`, pulsante **📍 Localizza via MyShipTracking** (`POST /api/ships/:mmsi/mstlocate`). Quando entrambe le fonti sono attive una nave stale viene interrogata su entrambe (a rotazione tra un giro e l'altro, non insieme); in mappa i marker MST sono **teal/ciano** per distinguerli dall'ambra di ShipFinder.
...
| **Logica badge / fallback** | `src/routes/ships.js` → `scrapeBadgeAt()`, `scrapeFallbackFix()` | Il fix scrapato viene mostrato (badge o marker) solo se l'AIS è stantio **e** lo scrape è più recente; un fix AIS fresco lo nasconde. Usata sia da `/ships/followed/active` sia da `/ships/active` (navi d'area). |
...
| **Ri-acquisizione stale via SF/MST** | `src/services/fallback-mode.js` → `sweep()`/`scrapeOne()` | Sweep periodico condiviso (vedi [Modalità fallback](#-modalità-fallback-per-area-silenziosa)): scrapa le seguite stale **e**, per le aree correntemente silenziose, le navi d'area — salva i fix con `source='sf'/'mst'`. |
...
[`src/services/fallback-mode.js`](../../src/services/fallback-mode.js) **non è più un interruttore globale**. Ogni area monitorata è indipendentemente "in fallback silenzioso" in base a tre fatti sulla riga `areas` corrispondente: `fallback_enabled` (bool, default 1, admin-configurabile per-area dalla schermata **Aree** — master on/off), `last_ais_message_at` (timestamp, aggiornato da [`ais-stream.js`](../../src/services/ais-stream.js) ad ogni messaggio AIS reale attribuito a quell'area, throttlato a 1 scrittura/minuto per area in memoria — mai una scrittura per-messaggio, per non ripetere l'esatto bug di starvation dell'event loop già preso e risolto per il ricalcolo banchine, vedi sotto) e `fallback_forced` (bool, default 0, admin-configurabile per-area — vedi sotto). "L'area X è silenziosa ora" si **calcola al volo** ad ogni sweep — `fallback_enabled AND (fallback_forced OR last_ais_message_at IS NULL OR più vecchio di AREA_SILENT_THRESHOLD_MIN)` — mai persistito come stato: niente `enter()`/`exit()`, niente isteresi di uscita (non serve: è silenzioso e a basso rischio, si autocorregge al primo messaggio reale, a meno che non sia forzata).

Questo è **volutamente scollegato** dal disservizio AIS globale descritto sopra ([`ais-uptime.js`](../../src/services/ais-uptime.js), invariato): quel rilevatore continua a **notificare soltanto** (Telegram/in-app "Disservizio AIS" a tutti gli utenti collegati, riferito all'intero servizio AISStream) — non attiva più nulla. Un'area può essere in fallback silenzioso per una copertura AISStream strutturalmente scarsa o assente (non un disservizio, un limite fisico della fonte per quella zona) senza che nessuno riceva una notifica: è il caso d'uso esplicito della feature.

- **Navi seguite, gestite incondizionatamente** — `fallback-mode.js` copre lo scraping SF/MST di **tutte** le navi seguite stantie ad ogni sweep, indipendentemente da qualsiasi area (una nave seguita non appartiene a una singola area). [`ship-follow.js`](../../src/services/ship-follow.js) ha perso il proprio ramo di scraping (`refresh()`'s `reacquireStaleViaShipfinder`/`reacquireStaleViaMst` sono spariti) — mantiene solo la WS dedicata/subscription-per-nave/world-box, che è ri-acquisizione AIS gratuita, non scraping. Motivo dell'accorpamento: un solo budget anti-ban condiviso by design (vedi sotto) — se i due moduli scrapassero la stessa nave in parallelo, il tetto "condiviso" verrebbe di fatto raddoppiato.
- **Navi d'area** — `db.getStaleAreaShips(freshMs, areaSilentCutoffIso)` fa JOIN `ships.last_area = areas.key`: solo le navi la cui area è correntemente silenziosa **e** abilitata entrano nel pool. Deliberatamente non riusa `ACTIVE_PREDICATE` (a differenza di `getActiveShips`): quel predicato è esattamente ciò che retrocede una nave da "attiva" a "passata" dopo `ACTIVE_WINDOW_HOURS`/`PORT_WINDOW_HOURS` di silenzio — usarlo anche qui creerebbe un catch-22 per un'interruzione più lunga di quella finestra (serve un fix scrapato per tornare "attiva", ma è esclusa dallo scraping perché non è più "attiva") che blocca per sempre il riposizionamento — bug reale osservato in produzione (AIS giù da giorni, scraping attivo, mappa vuota) e origine di questo intero redesign.
- **Scoperta navi mai viste da AIS, via arrivi/partenze porto** — per un'area silenziosa con almeno un porto **confermato** e **risolto su MyShipTracking** (`area_ports.status='confirmed' AND mst_pid IS NOT NULL` — la risoluzione gira già al boot/creazione/modifica area via `resolveMstPidForConfirmedPorts`, in `port-discovery.js`, prima un campo scritto ma mai consumato), `sweepPortDiscovery()` interroga `crawlPortArrivals(pid)` (`scrapers/myshiptracking.js`, prima codice morto senza chiamanti) con una cadenza propria di 30min/porto (mappa in-memory `lastPortPollAt`, non nel DB — stesso motivo del throttle su `last_ais_message_at`). Per ogni `{mmsi, name}` mai visto: `db.ensureShipStub` (la stessa primitiva già usata per le navi seguite scoperte solo via scraping) + `db.setShipLastArea` + `enrichment.enrichNewShip`. **Nessuna scrape di posizione qui**: la nave stubbata (sentinella `NEVER_SEEN_AIS`) entra da sola, al giro successivo, nel pool di riposizionamento normale — zero doppio conteggio sul budget, `scrapeOne()` resta invariato. Limite intrinseco, non risolvibile via software: funziona solo per aree con un porto riconoscibile nel bbox (il caso quasi sempre vero per le aree di questa app) — non scopre navi in mare aperto senza porto, e non può "inventare" un MMSI mai esistito da nessuna parte. Nessuna integrazione fa ricerca-nave-per-bbox (verificato leggendo il codice: VF/MT/GFW/SF/MST richiedono tutte un MMSI/IMO già noto; la funzione bbox di GFW, `getAnchoragesInBbox`, è uno stub morto — 404/403 confermati contro l'API pubblica col token attuale) — `crawlPortArrivals` è l'unica eccezione, ed è per-porto, non per-bbox.
- **Sospensione VF/MT, per-nave/per-area** — [`enrichment.js`](../../src/services/enrichment.js) sospende VesselFinder/MarineTraffic (`enrichNewShip`, `enrichAllExisting`, `enrichActiveShips`) solo per le navi la cui **propria** area è correntemente silenziosa (`isAreaSilentForShip(ship)`), non più con un interruttore globale: una nave in un'area sana continua a ricevere VF/MT anche se un'ALTRA area è silenziosa nello stesso momento. Non danno coordinate (prodotto a pagamento) e MarineTraffic è la sorgente più fragile (bypass Cloudflare via `node-libcurl`); il check duplica la logica di silenzio invece di chiamare `fallback-mode.js`, per evitare un require-cycle (il sweep di scoperta-porto chiama a sua volta `enrichment.enrichNewShip`).
- **Forzatura manuale, per-area** — `fallback_forced` (colonna `areas`, default 0) fa contare un'area come "in fallback" anche mentre l'AIS le sta mandando messaggi normalmente: pensata per una bbox con copertura AIS reale strutturalmente scarsa (poche navi con transponder), dove la soglia di silenzio non scatterebbe mai da sola. Toccano solo `getStaleAreaShips`/`getPortDiscoveryTargets` (SQL, `OR a.fallback_forced = 1`) e `enrichment.js`'s `isAreaSilentForShip` — lo stream AIS per l'area resta sempre attivo e ascoltato, invariato: la forzatura aggiunge scraping in parallelo, non lo sostituisce mai. `getAreaStatuses()` distingue `silentSince` (solo per silenzio genuino, `last_ais_message_at`) da `fallbackForced` (bool separato) — un'area forzata-ma-fresca non ha un "silenzioso da" reale, quindi la UI non ne inventa uno.
- **Pannello admin e UI, tutto admin-only** — due interruttori indipendenti per riga nella schermata **Aree** (`PATCH /areas/:key/fallback`, body `{enabled}` e/o `{forced}`): master on/off e ⚡ forza (visibile solo se il master è acceso). Badge di stato live per riga — "🟢 AIS attivo" o "🔴 In fallback" (con durata se il silenzio è reale, "(forzata)" altrimenti) — più ⚠/ℹ se il porto non è risolto su MST (il riposizionamento delle navi già note funziona comunque, solo la scoperta di navi nuove non è attiva). Impostazioni → **Diagnostica AIS** mostra la stessa lista per-area al posto del vecchio blocco unico globale, più circuit breaker e storico volume scraping (aggregati, non per-area). Il server filtra `fallbackEnabled`/`fallbackForced`/`portArrivalsReady`/il dettaglio per-area sia su `GET /areas` sia su `GET /api/stream/status`/`health` per i non-admin (`GET /api/stream/health` ora `requireAdmin`). La pagina **"Navi presenti"** mostra il nome dell'area corrente + un badge 📡/🔀 (solo admin) che indica se i dati di quell'area vengono da AIS o da fallback in questo momento.
- **Pannello Porti (schermata Aree, `⚓`)** — [`public/js/areas.js`](../../public/js/areas.js) `openPortsPanel`, pannello **inline** sotto la mappa (non un modale: la mappa con i marker resta visibile mentre si legge la lista — un primo tentativo con `#modal-overlay` copriva la mappa che avrebbe dovuto mostrare l'evidenziazione, corretto prima del rilascio). Tabella porti (`GET /areas/:key/ports`) con conferma/rifiuta (`POST /areas/:key/ports/:id/confirm|reject`) invariati. **Nuovo**: sezione "Confronta con sorgenti esterne", sempre disponibile (a differenza della scoperta automatica, non condizionata da banchine già osservate) — `GET /areas/:key/ports/search-external/stream` (SSE, `requireAdmin`) chiama `portDiscovery.searchExternalCandidates(areaKey, onCandidate, shouldStop)`, una cascata locode/WPI/GFW/VesselFinder **estratta** da `discoverPortsForArea` in una funzione riusabile che invoca `onCandidate` per ogni risultato grezzo (non clusterizzato: qui decide un umano, non serve deduplicare) man mano che arriva — locode/WPI/GFW quasi istantanei, VesselFinder al ritmo paced esistente (jitter, fino a 20 lookup). `shouldStop` (poll tra un lookup VF e l'altro) evita di continuare a bruciare budget anti-ban se l'admin chiude la finestra a metà ricerca. **Deliberatamente senza il gate `isAnyAreaSilent()`** che invece protegge `discoverPortsForArea`: quel gate esiste solo perché il flusso completo chiama anche `resolveMstPidForConfirmedPorts` (MST, condiviso col budget di `fallback-mode.js`) subito dopo — `searchExternalCandidates` da sola non tocca mai SF/MST, quindi gatearla allo stesso modo bloccherebbe la funzione globalmente ogni volta che un'area qualunque è silenziosa/forzata, per una risorsa che non condivide. Frontend: layer mappa dedicato (marker blu tratteggiati) accanto a quello dei porti confermati, lista con checkbox popolata via `insertAdjacentHTML` (non un re-render completo, altrimenti ogni candidato in arrivo deselezionerebbe le caselle già spuntate durante una ricerca che può durare oltre un minuto), `POST /areas/:key/ports/candidates` (`{candidates:[{name,lat,lon,source}]}`, max 200) aggiunge diretto a `confirmed`/`admin_reviewed=1` via `db.addManualAreaPort` — la scelta dell'admin è già la revisione, a differenza dell'upsert automatico che passa da `review` quando una sola fonte concorda.
- **Bug corretto — click sullo slider inghiottito dalla riga** — la delega click sulla tabella Aree usava `e.target.closest('.area-fallback-toggle')` per non aprire l'editor riga quando si cliccava un toggle; ma cliccando lo `<span class="toggle-slider">` visibile (fratello, non antenato, dell'`<input>` con quella classe) il target non risale mai a un antenato con quella classe, quindi il click "sfuggiva" al guardiano e apriva comunque l'editor — il toggle sembrava non rispondere (in realtà l'azione partiva, ma la UI saltava altrove subito dopo). Fix: guardia sull'intero `label.toggle` invece del solo `.area-fallback-toggle`/`.area-fallback-force-toggle`, così qualunque elemento cliccato dentro l'etichetta (span, label o input) viene correttamente riconosciuto.
- **Pulizia porti duplicati auto-generati** — `db.deleteUnreviewedBerthPorts(areaKey)`, chiamata da `discoverPortsForArea` prima di ri-clusterizzare le banchine: cancella le righe `area_ports` di quell'area con `sources = '["berths"]'` esatto e `admin_reviewed = 0`. Bug reale corretto: `upsertAreaPort` fa `ON CONFLICT(area_key, name)` — se un cluster successivo sceglie un nome-rappresentante diverso (centroide leggermente spostato, un membro in più/meno), la vecchia riga non veniva mai sostituita, solo affiancata; su un'area rilanciata più volte nel tempo si accumulavano decine di righe quasi-duplicate mai riviste da un admin (osservato in produzione: stesso punto salvato 3-4 volte con nomi diversi). Righe confermate/rifiutate a mano o corroborate da una fonte esterna non vengono mai toccate. Il nome di fallback per una banchina senza nome è stato anche **spogliato della parola italiana hardcoded** (`Porto {lat},{lon}` → solo le coordinate): un nome persistito nel DB non può passare dall'i18n del client a runtime.
- **Log live delle chiamate** ("🔀 Log modalità fallback", visibile solo agli admin, sopra "Log attività" in sidebar) — finestra flottante che mostra, in tempo reale, le chiamate a ShipFinder/MyShipTracking man mano che avvengono, indipendentemente da quale area/nave le ha innescate. Meccanismo identico al log applicativo (`app-log.js`/`realtime.js`): `fallback-mode.js`'s `scrapeOne()` chiama `broadcastFallbackScrape({ts,source,mmsi,ok})` (in `realtime.js`, stesso pattern Set-di-client-SSE + `res.write` di `broadcastAppLog`) dopo ogni tentativo, riuscito o no; `GET /api/fallback-scrape/stream` (SSE, `requireAdmin`, `routes/fallback-scrape.js`) fan-out ai client connessi, `GET /api/fallback-scrape` backfilla con un buffer in-memory delle ultime 200 chiamate (`fallbackMode.getRecentScrapeEvents()`, stesso tradeoff in-memory-only del circuit breaker). Frontend (`public/js/fallback-scrape-log.js`): un `EventSource` per finestra, **grafico a linea SVG** (non più a barre impilate — con bucket da 30s su una serie perlopiù vuota le barre sembravano disallineate/casuali; una linea con marker a pallino sui punti non-zero si legge meglio) per riuscite/fallite con legenda colore, bucket di 30s sugli ultimi 10 minuti, ricalcolato ad ogni evento — a differenza del grafico storico a 48h in Diagnostica AIS (bucket orari, dati da `scrape_log`, rimasto a barre), questo è push-based e a grana fine, pensato per "sto scrapando adesso, sta andando bene?" non per lo storico. Le righe di log applicativo (`Fallback: posizione ShipFinder/MyShipTracking per <nave>`) riportano ora anche **area e porto più vicino** tra parentesi quadre quando la nave appartiene a un'area (`areaPortLogTag` in `fallback-mode.js`, ricerca il porto confermato più vicino entro 8km via lo stesso `haversineM` di `ship-analysis.js`) — assente per le navi seguite, che non hanno un'area.
- **Anti-ban, in ordine di importanza** (invariato — un solo budget/circuit breaker condiviso da navi seguite, navi d'area **e** scoperta-porto, più aree silenziose insieme ridistribuiscono lo stesso tetto invece di sommarlo):
  - **Budget globale orario** (`FALLBACK_MAX_REQ_PER_HOUR`, default 90, SF+MST insieme) con **coda a priorità**: le navi non rivisitate da più tempo vengono scrapate per prime, invece del cap fisso per-sweep di oggi.
  - **Rotazione sorgente per nave** — round-robin SF/MST invece di interrogare sempre entrambe per la stessa nave, dimezzando il carico per sorgente a parità di copertura.
  - **Jitter** — stagger randomizzato (1.5–4.5s) tra le richieste invece del ritardo fisso di 2s di oggi, per non avere un pattern temporale riconoscibile.
  - **Rotazione User-Agent** — pool di UA desktop realistici (`scrapers/http.js` → `pickUA()`), scelto per richiesta invece di un'unica stringa fissa.
  - **Circuit breaker per sorgente** — `http.js` ora classifica gli errori HTTP (`classifyFailure`, con parsing di `Retry-After`); `fallback-mode.js` traccia i fallimenti 403/429 **su navi distinte** in una finestra scorrevole (`FALLBACK_CIRCUIT_TRIP_COUNT` su `FALLBACK_CIRCUIT_TRIP_WINDOW_MIN` minuti) e, superata la soglia, **sospende l'intera sorgente** per `FALLBACK_CIRCUIT_COOLDOWN_MIN` minuti — un salto rispetto alla cache negativa per-nave già esistente, che non aggrega mai i fallimenti a livello di sorgente. Stato **solo in-memory** (non in `meta`/DB): un riavvio che lo azzera è un compromesso accettabile rispetto alla complessità di persisterlo/scadenzarlo correttamente.
- **Alert "sospetto ban"** — l'apertura/chiusura di un circuito genera un alert **solo per gli admin** (`db.getAdminUserIds()`): notifica in-app (tipo `suspected_ban`/`suspected_ban_cleared`, riusa la tabella `notifications` esistente) + Telegram (`telegram.broadcastAdminAlert`, toggle personale `telegramNotifySuspectedBan`) + banner in-app (estende il banner di disservizio AIS esistente con una riga visibile solo se `me.isAdmin`). Bypassa `notify-categories.js` come l'evento `outage`, essendo un evento di sistema non legato a una singola nave.
- **Migrazione minima, backup verificato** — nessuna tabella nuova: tre colonne su `areas` (`fallback_enabled INTEGER DEFAULT 1`, `last_ais_message_at TEXT`, `fallback_forced INTEGER DEFAULT 0`, via il solito loop `ALTER TABLE ... ADD COLUMN` try/catch), e `areas` è già in `BACKUP_TABLES`. Verificato empiricamente (non solo per costruzione) che un backup pre-feature, privo di queste colonne, restora con `fallback_enabled=1`/`last_ais_message_at=NULL`/`fallback_forced=0` — esattamente i default sicuri, grazie a come `restoreFrom` copia per intersezione di colonne. Le vecchie righe `meta` `fallback_mode_active`/`fallback_mode_since` di un backup pre-feature restano orfane e innocue: nessun codice le legge più.
- **`outage.since` da fonte esterna autorevole, non da rilevazione locale** — `probeOne()` legge anche `lastMessageReceived` dalla risposta del monitor di uptime (l'ultimo messaggio visto **dal monitor**, non da noi), usato come `since` quando disponibile — fallback su `meta` (`ais_outage_since`, persistito per sopravvivere ai riavvii) e infine su "adesso" se nessuno dei due è disponibile. Così un **primo deploy fatto giorni dentro un disservizio già in corso** mostra da subito le ore di down reali nel banner/nelle notifiche, invece di far ripartire il conto da zero a ogni deploy.
- **Riavvio a metà disservizio: banner e verdetto senza attesa** — `outage.serviceDown`/`monitorState`/`monitorSource` sono rehydratati da `meta` all'avvio del modulo (oltre a `since`, già persistito), quindi il banner mostra subito l'ultimo stato noto invece di sparire finché `evaluate()` non lo ricalcola. Un riavvio azzera anche il **silenzio locale** dello stream (si riconnette da zero): senza correttivo, `evaluate()` avrebbe dovuto aspettare di nuovo `AIS_OUTAGE_SILENCE_MIN` minuti di silenzio prima anche solo di ricontattare il monitor, lasciando il banner vuoto/stale nel frattempo. `init()` chiama quindi `reconfirmOnBoot()` una tantum quando lo stato rehydratato dice "era giù": un probe immediato al monitor (rispetta comunque `MIN_PROBE_GAP_MS` per i check successivi) che conferma l'outage in corso (nessuna nuova notifica, non è una nuova rilevazione) o lo chiude subito se si era risolto durante il riavvio. `evaluate()` e `reconfirmOnBoot()` condividono la stessa logica di transizione (`applyDown`/`applyUp`) per non far divergere la catena di fallback di `since` tra i due punti di chiamata. Questa logica è **indipendente** dalla modalità fallback per-area (sopra): riguarda solo banner/notifiche del disservizio globale.
...
> **Bug corretto — `data-tip` inerte fuori dalla whitelist**: `initGlossaryTooltip()` aggancia gli eventi `mouseover`/`mouseout` **solo** agli elementi che matchano `TIP_SELECTOR`, una whitelist esplicita di selettori CSS — un attributo `data-tip` su un elemento **non incluso** in quella lista è semplicemente ignorato, senza errori, senza fallback al `title` nativo: sembra un tooltip "che non fa niente". I badge/toggle della schermata Aree (dot live 🟢/🔴, badge porto ℹ/⚠, e i due `<label class="toggle">` per riga) erano stati costruiti con `data-tip` seguendo il pattern esistente ma **senza** essere aggiunti a `TIP_SELECTOR` — invisibili al passaggio del mouse fin dal primo commit, non una regressione. Fix: `.area-fallback-live[data-tip], .area-badge-warn[data-tip], .area-badge-info[data-tip], .toggle[data-tip]` aggiunti alla whitelist in `main.js`. `.toggle` è generico (usato da ogni interruttore on/off dell'app) ma il selettore richiede comunque `[data-tip]`, quindi scatta solo dove l'attributo è presente — riusabile per qualunque futuro tooltip su un toggle, non solo questi due.
...
- **Toggle per categoria** — indipendenti dalle notifiche in-sidebar (un utente può ricevere una categoria su Telegram anche con quella in-app spenta, e viceversa). Master per-utente `telegramEnabled` + sette categorie: score alto, rientro nave, cambio area, nuova banchina, caratterizzazione banchina, **disservizio AIS** (outage, evento globale verso tutti gli utenti collegati col toggle attivo) e **avvio/stop monitoraggio area** (quando l'utente aggiunge/rimuove una propria area) — più **sospetto ban** (`telegramNotifySuspectedBan`, evento di sistema come l'outage ma inviato **solo agli admin**, vedi [Modalità fallback](#-modalità-fallback-per-area-silenziosa)) e altre **sei categorie** per le [notifiche "Attività del gruppo"](#-gruppi-di-utenti) (`telegramNotifyGroupArea` ecc., solo per chi è in un gruppo). Persistiti come preferenze per-utente (`telegramNotify*`). Le quattro categorie **legate a una nave** (score alto, rientro, cambio area, rendezvous) seguono **anche** il filtro per tipo nave e il flag "vista" del tab Notifiche (vedi sopra) — sono lo stesso gate della notifica in-app, non un filtro separato; nuova banchina/caratterizzazione/outage/avvio-stop monitoraggio/sospetto ban non sono legate a una nave e non sono filtrate.
